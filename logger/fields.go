...
const (
	...
	// TraceE is a standard prefix for log messages that end a trace.
	TraceE = "TRACE->E: "
)
...
func NewOperation(log *zap.Logger, msg, name string, fields ...zapcore.Field) (*zap.Logger, func()) {
	...
	now := time.Now()
	log = log.With(f...)
	log.Info(TraceS+msg, OperationEventStart())

	return log, func() { log.Info(TraceE+msg, OperationEventEnd(), OperationElapsed(time.Since(now))) }
}
