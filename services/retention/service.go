...
func (s *Service) Open() error {
	...
	s.logger.Info("Starting retention policy enforcement service",
		zap.Duration("checkInterval", time.Duration(s.config.CheckInterval)))
...
func (s *Service) run() {
	...
	for {
		...
		case <-ticker.C:
			...
			if retryNeeded {
				s.logger.Info("One or more errors occurred during shard deletion and will be retried on the next check", zap.Duration("checkInterval", time.Duration(s.config.CheckInterval)))
			}
		}
	}
}
