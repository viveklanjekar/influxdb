...
func (s *Service) handleTelnetConn(conn net.Conn) {
	...
	for {
		...
		if len(inputStrs) < 4 || inputStrs[0] != "put" {
			...
			if s.LogPointErrors {
				s.Logger.Info("Malformed line", zap.String("line", line), zap.String("remoteAddr", remoteAddr))
			}
			continue
		}
		...
		if err != nil {
			...
			if s.LogPointErrors {
				s.Logger.Info("Malformed time", zap.String("time", tsStr), zap.String("remoteAddr", remoteAddr))
			}
		}
		...
		default:
			atomic.AddInt64(&s.stats.TelnetBadTime, 1)
			if s.LogPointErrors {
				s.Logger.Info("Time must be 10 or 13 chars", zap.String("time", tsStr), zap.String("remoteAddr", remoteAddr))
			}
			continue
		}
		...
		for t := range tagStrs {
			...
			if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
				...
				if s.LogPointErrors {
					s.Logger.Info("Malformed tag data", zap.String("tag", tagStrs[t]), zap.String("remoteAddr", remoteAddr))
				}
				continue
			}
			k := parts[0]
		...
		if err != nil {
			...
			if s.LogPointErrors {
				s.Logger.Info("Bad float", zap.String("value", valueStr), zap.String("remoteAddr", remoteAddr))
			}
			continue
		}
		fields["value"] = fv
